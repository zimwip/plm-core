...
/**
 * Polls {@code event_outbox} and delivers pending events via NATS, then deletes them.
 * Uses {@code FOR UPDATE SKIP LOCKED} so multiple dst replicas never double-deliver.
 */
@Slf4j
@Component
@ConditionalOnProperty(prefix = "plm.nats", name = "enabled", havingValue = "true")
public class DstOutboxPoller {

    private final DSLContext dsl;
    private final PlmMessageBus messageBus;
    private DSLContext quietDsl;

    public DstOutboxPoller(DSLContext dsl, PlmMessageBus messageBus) {
        this.dsl = dsl;
    ...
    public void poll() {
        ...
        log.debug("DstOutboxPoller delivered {} event(s)", pending.size());
    }
}
