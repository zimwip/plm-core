...
function getLinkLayer(link) {
  return (link.linkAttributeValues || []).find(av => av.attributeId === 'layer')?.value || 'main';
}
...
function isRepresentationLink(link) {
  if (link.targetSourceCode !== 'DATA_LOCAL') return false;
  const kind = getLinkKind(link);
  if (kind === 'simplified' || kind === 'design' || kind === 'original') return true;
  return isStepLink(link); // legacy links without kind attribute
}
...
// Row-major 4×4 matrix multiply: C = A × B (both arrays of 16 doubles).
...
async function collectStepNodes(api, userId, nodeId, nodeLabel, stateColor, links, depth, maxDepth, visited, stateColorMap, parentLinkId = null, worldMatrix = null) {
  if (depth > maxDepth) return [];
  // At root (depth=0, parentLinkId=null) guard on nodeId to prevent root cycles.
  // For deeper levels, guard is per-linkId (allows multi-instance of same nodeId).
  if (depth === 0 && parentLinkId === null && visited.has(nodeId)) return [];
  if (depth === 0 && parentLinkId === null) visited.add(nodeId);

  // Group DATA_LOCAL representation links by layer, prefer simplified over design per layer
  const repLinks = links.filter(isRepresentationLink);
  const byLayer = new Map();
  for (const l of repLinks) {
    const layer = getLinkLayer(l);
    const kind  = getLinkKind(l);
    if (!byLayer.has(layer)) byLayer.set(layer, { simplified: null, fallback: null });
    const entry = byLayer.get(layer);
    if (kind === 'simplified') entry.simplified = l;
    else if (!entry.fallback)  entry.fallback   = l; // design / original / legacy
  }
  const directParts = [];
  for (const [layer, { simplified, fallback }] of byLayer) {
    if (layer !== 'main') continue; // only main layer for 3D preview
    const l = simplified || fallback;
    if (!l) continue;
    const kind = getLinkKind(l) || 'design';
    directParts.push({
      uuid: l.targetKey,
      fileName: l.displayKey || l.targetKey,
      sizeBytes: l.targetDetails?.sizeBytes,
      instanceKey: parentLinkId ? `${l.targetKey}#${parentLinkId}` : l.targetKey,
      matrix: worldMatrix,
      kind,
    });
  }
  ...
  if (directParts.length > 0) {
    result.push({ nodeId, nodeLabel, stateColor, depth, parts: directParts, instanceId: parentLinkId || nodeId });
  }
  ...
  if (depth < maxDepth) {
    ...
    await Promise.all(selfLinks.map(async link => {
      ...
      if (!linkId || visited.has(linkId)) return;
      visited.add(linkId);
      try {
        // Extract local matrix from link attribute values
        const posValue = (link.linkAttributeValues || []).find(av => av.attributeId === 'position')?.value;
        let localMatrix = null;
        if (posValue) {
          const parts = posValue.split(',').map(Number);
          if (parts.length === 16 && parts.every(n => !isNaN(n))) localMatrix = parts;
        }

        // Compose world matrix: worldMatrix × localMatrix
        let composedMatrix = null;
        if (worldMatrix && localMatrix) {
          composedMatrix = multiplyMatrix4(worldMatrix, localMatrix);
        } else if (localMatrix) {
          composedMatrix = localMatrix;
        } else if (worldMatrix) {
          composedMatrix = worldMatrix;
        }

        const childLinks = await api.getChildLinks(null, link.targetNodeId);
        const childColor = stateColorMap?.[link.targetState] || '#6b7280';
        const childNodes = await collectStepNodes(
          api, userId, link.targetNodeId,
          link.targetLogicalId || link.targetNodeId,
          childColor,
          Array.isArray(childLinks) ? childLinks : [],
          depth + 1, maxDepth, new Set(visited), stateColorMap,
          parentLinkId ? `${parentLinkId}/${linkId}` : linkId, composedMatrix
        );
...
export default function NodeEditor({
  ...
  const [selectedParentLinkId, setSelectedParentLinkId] = useState(null);
  ...
  const [desc, setDesc] = useState(() => itemData?.data ?? null);
  useEffect(() => {
  ...
  const descVersionId = desc?.metadata?.currentVersionId;
  useEffect(() => {
    if (desc?.metadata?.violations) setSaveViolations(desc.metadata.violations);
  ...
  useEffect(() => {
    ...
    const rootLabel = desc?.metadata?.logicalId || desc?.title || nodeId;
    const rootColor = stateColorMap?.[desc?.metadata?.state] || '#6b7280';
    collectStepNodes(psmApi, userId, nodeId, rootLabel, rootColor, children, 0, 3, new Set(), stateColorMap, null, null)
  ...
  const load = useCallback(async () => {
    try {
      const [h, c, sigs] = await Promise.all([
        psmApi.getVersionHistory(userId, nodeId).catch(() => []),
        psmApi.getComments(userId, nodeId).catch(() => []),
        psmApi.getSignatureHistory(userId, nodeId).catch(() => []),
      ]);
  ...
  const refreshCommentCounts = useCallback(async () => {
    try {
      const c = await psmApi.getComments(userId, nodeId).catch(() => []);
      const counts = {};
      if (Array.isArray(c)) {
        c.forEach(comment => {
          const vid = comment.versionId;
  ...
  async function openDiff(v2Num) {
    ...
    setDiffLoading(true);
    try {
      const data = await psmApi.getVersionDiff(userId, nodeId, v1Num, v2Num);
  ...
  async function openLinkPanel(preselect = null) {
    ...
    try {
      const [lts, srcs] = await Promise.all([
        api.getNodeTypeLinkTypes(userId, desc?.metadata?.nodeTypeId).catch(() => []),
        psmApi.getSources(userId).catch(() => []),
      ]);
  ...
  async function loadKeySuggestions(sourceId, type, query) {
    try {
      const out = await psmApi.getSourceKeys(userId, sourceId, type, query, 25);
  ...
  async function handleCreateLink() {
    ...
    try {
      ...
      const createLinkAction = desc.actions?.find(a => a.code === 'create_link');
      ...
      await (createLinkAction.path
        ...
        : authoringApi.executeAction(nodeId, createLinkAction.code, userId, activeTxId, createLinkParams));
  ...
  async function handleUpdateLink(linkId, newLogicalId, newTargetKey, linkAttrs) {
    const updateLinkAction = desc.actions?.find(a => a.code === 'update_link');
    ...
    try {
      ...
      await (updateLinkAction.path
        ...
        : authoringApi.executeAction(nodeId, updateLinkAction.code, userId, activeTxId, updateLinkParams));
      setEditingLinkId(null);
      await refreshTx();
      setPbsLoaded(false);
      await Promise.all([
        psmApi.getChildLinks(userId, nodeId).then(c => setChildren(Array.isArray(c) ? c : [])),
        psmApi.getParentLinks(userId, nodeId).then(p => setParents(Array.isArray(p) ? p : [])),
      ]);
      setPbsLoaded(true);
    } catch (e) { toast(e, 'error'); }
  ...
  async function handleDeleteLink(linkId) {
    const deleteLinkAction = desc.actions?.find(a => a.code === 'delete_link');
    ...
    try {
      ...
      await (deleteLinkAction.path
        ...
        : authoringApi.executeAction(nodeId, deleteLinkAction.code, userId, activeTxId, { linkId }));
      setSelectedLinkId(id => id === linkId ? null : id);
      await refreshTx();
      setPbsLoaded(false);
      await Promise.all([
        psmApi.getChildLinks(userId, nodeId).then(c => setChildren(Array.isArray(c) ? c : [])),
    ...
    try {
      ...
      const result = action.path
        ...
        : await authoringApi.executeAction(
            nodeId, action.code, userId, txId, params, action.metadata?.transitionId);
  ...
  function triggerAction(action) {
    ...
    if (uiParams.length > 0) {
      const defaults = {};
      uiParams.forEach(p => { if (p.defaultValue) defaults[p.name] = p.defaultValue; });
      setActionParams(defaults);
      setActionDialog(action);
    } else if (action.metadata?.displayCategory === 'DANGEROUS') {
  ...
  async function autoSave(pendingEdits, currentTxId, updateAction) {
    ...
    try {
      const result = await (updateAction?.path
        ...
        : authoringApi.executeAction(nodeId, updateAction?.code ?? updateAction, userId, currentTxId, saveParams));
      patchNodeDescAttrs(pendingEdits);
      setEdits({});
  ...
  useEffect(() => {
    ...
    setHistLoading(true);
    psmApi.getNodeDescription(userId, nodeId, null, viewVersionNum)
  ...
  const isOpenVersion = activeDesc?.metadata?.txStatus === 'OPEN';
  ...
  const fingerprintChanged = activeDesc?.metadata?.fingerprintChanged;
  ...
  const updateNodeAction = allActions.find(a => a.code === 'update_node' && a.metadata?.authorized !== false);
  // Visible header actions: authorized, non-internal, non-structural
  const headerActions = allActions.filter(a =>
    a.metadata?.authorized !== false && !INTERNAL_ACTIONS.has(a.code) && a.metadata?.displayCategory !== 'STRUCTURAL' && a.metadata?.displayCategory !== 'PROPERTY'
  );
  const propertyActions = allActions.filter(a =>
    a.metadata?.authorized !== false && a.metadata?.displayCategory === 'PROPERTY'
  );
  ...
  const guardTooltip = (action) => {
    ...
    if (!v?.length) return '';
    return 'Blocked:\n\u2022 ' + v.map(x => typeof x === 'string' ? x : x.message || x.code).join('\n\u2022 ');
  };
  // Transition actions have code "transition|<uuid>" — match by prefix
  const allTransitions    = allActions.filter(a => a.code?.startsWith('transition'));
  const transitionGuardViolations = new Map(
    allTransitions
      .filter(a => a.guardViolations?.length > 0)
      .map(a => [a.label, a.guardViolations])
  );
  // Available (unblocked) transition names for lifecycle diagram
  const transitions = headerActions.filter(a => a.code?.startsWith('transition'));
  const canUpdateLink     = activeDesc?.actions?.some(a => a.code === 'update_link');
  const canDeleteLink     = activeDesc?.actions?.some(a => a.code === 'delete_link');
  const checkoutAction    = allActions.find(a => a.code === 'checkout');
  ...
  const currentVersionNum = history.find(
    v => (v.id || v.ID) === desc?.metadata?.currentVersionId
  )?.version_number ?? null;
      ...
      <div className="node-header">
        <div className="node-title-group">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span className="node-identity">{activeDesc.metadata?.logicalId || activeDesc.title}</span>
            {activeDesc.subtitle && (
              <span className="node-display-name">{activeDesc.subtitle}</span>
            )}
            <span style={{
              ...
              {viewVersionNum && '🕐 '}
              {activeDesc.metadata?.iteration === 0 ? activeDesc.metadata?.revision : `${activeDesc.metadata?.revision}.${activeDesc.metadata?.iteration}`}
            </span>
            <StatePill stateId={activeDesc.metadata?.state} stateName={activeDesc.metadata?.stateName} stateColorMap={stateColorMap} />
            {!viewVersionNum && desc.metadata?.lock?.locked && (
              <span className="pill" style={{ color: 'var(--muted)', background: 'rgba(100,116,139,.1)', border: '1px solid rgba(100,116,139,.2)' }}>
                🔒 {desc.metadata?.lock?.lockedBy}
              </span>
            )}
          </div>
          <div className="node-meta">
            {isOpenVersion && activeDesc?.metadata?.lock?.lockedBy === userId && (
            ...
            {isOpenVersion && activeDesc?.metadata?.lock?.lockedBy && activeDesc?.metadata?.lock?.lockedBy !== userId && (
              <span style={{ fontSize: 11, color: 'var(--accent)', fontStyle: 'italic', opacity: 0.9 }}>
                ✎ in progress — being edited by {activeDesc.metadata?.lock?.lockedBy}
              </span>
            )}
            {saveStatus === 'saving' && (
        ...
        <div className="node-actions">
          {headerActions.map(a => {
            ...
            const tooltip   = isBlocked ? guardTooltip(a) : (a.description || a.label);
            const color     = a.metadata?.displayColor; // handler-provided color override
            const catClass  = color ? ''
                            : a.metadata?.displayCategory === 'DANGEROUS' ? 'btn-danger'
                            : a.metadata?.displayCategory === 'PRIMARY'   ? 'btn-success'
                            : '';
            ...
            return (
              <button
                key={a.code}
                className={`btn btn-sm ${catClass}`}
                disabled={loading || isBlocked}
                title={tooltip}
                style={{ ...colorStyle, ...(isBlocked ? { opacity: 0.45, cursor: 'not-allowed' } : {}) }}
                onClick={() => !isBlocked && triggerAction(a)}
              >
                {isBlocked ? `✕ ${a.label}` : a.label}
              </button>
            );
          })}
        </div>
      ...
      {actionDialog && ReactDOM.createPortal(
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 16 }}>{actionDialog.label}</div>
            {uploadProgress !== null && (
            ...
            {(actionDialog.parameters || []).filter(p => p.widget).map(p => {
              ...
              if (p.options?.length > 0) {
                enumValues = p.options; // already [{value, label}] from ActionParameter.Choice
              }
              return (
                <div className="field" key={p.name} style={{ marginBottom: 14 }}>
                  ...
                  {p.widget === 'FILE' ? (
                    <>
                      ...
                      />
                      {p.hint && <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>{p.hint}</div>}
                    </>
                  ) : enumValues ? (
                    <select
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                      ...
                      />
                      {p.hint && <span style={{ fontSize: 12, color: 'var(--muted)' }}>{p.hint}</span>}
                    </label>
                  ) : (
                    <input
                      className="field-input"
      ...
      {viewVersionNum && (
        <div style={{
          ...
          <span style={{ color: '#92400e' }}>
            ...
            {historicalDesc && ` (${historicalDesc.metadata?.iteration === 0 ? historicalDesc.metadata?.revision : `${historicalDesc.metadata?.revision}.${historicalDesc.metadata?.iteration}`})`}
            {histLoading && ' — loading…'}
            {' · read-only'}
          </span>
            ...
            <div>
              ...
              <div className="attr-grid">
                ...
                <div className="field">
                  ...
                  <input
                    className="field-input"
                    value={externalIdEdit !== null ? externalIdEdit : (activeDesc.metadata?.externalId || '')}
                    placeholder="—"
                    onChange={e => setExternalIdEdit(e.target.value)}
                    onFocus={() => setExternalIdEdit(activeDesc.metadata?.externalId || '')}
                    onBlur={async () => {
                      if (externalIdEdit === null) return;
                      const val = externalIdEdit.trim();
                      if (val !== (activeDesc.metadata?.externalId || '')) {
                        await psmApi.updateExternalId(userId, nodeId, val).catch(() => {});
                        await refreshNodeDesc();
                      }
                      setExternalIdEdit(null);
                    }}
                  />
                ...
                <div className="field">
                  ...
                  <input className="field-input" readOnly value={activeDesc.metadata?.technicalId || ''} title={activeDesc.metadata?.technicalId || ''} />
                </div>
                <div className="field">
                  <label className="field-label">Creator</label>
                  <input className="field-input" readOnly value={activeDesc.metadata?.createdBy || ''} />
                </div>
                <div className="field">
                  <label className="field-label">Created</label>
                  <input className="field-input" readOnly value={fmtDate(activeDesc.metadata?.createdAt)} />
                </div>
                <div className="field">
                  <label className="field-label">Modified by</label>
                  <input className="field-input" readOnly value={activeDesc.metadata?.modifiedBy || ''} />
                </div>
                <div className="field">
                  <label className="field-label">Last update</label>
                  <input className="field-input" readOnly value={fmtDate(activeDesc.metadata?.lastUpdate)} />
                </div>
              </div>
            </div>
      ...
      {activeSubTab === 'pbs' && (
        <div
          ...
          onDrop={e => {
            ...
            clearDraggedNode();
            if (!data) return;
            if (!desc?.actions?.some(a => a.code === 'create_link')) {
        ...
        >
          ...
          {/* Add link button */}
          {activeDesc.actions?.some(a => a.code === 'create_link') && (
                          ...
                          return (
                            ...
                            >
                              ...
                              {isSelf ? (
                                ...
                                <>
                                  <td style={{ color: 'var(--muted)', fontSize: 12 }}>{c.targetNodeType || <span style={{ opacity: .35 }}>—</span>}</td>
                                  <td style={{ fontFamily: 'var(--sans)', fontSize: 12 }}>
                                    {isEditing ? (
                                      <div style={{ position: 'relative' }}>
                                        <input
                            ...
                            {selectedLinkId === c.linkId && !isEditing && (() => {
                              ...
                              return (
                                <tr className="link-detail-expand" onClick={e => e.stopPropagation()}>
                                  <td colSpan={colCount}>
                                    <div className="link-detail-inner">
                                      {ltAttrs.length === 0 ? (
                                        <span style={{ fontSize: 11, opacity: .5 }}>No attributes defined for this link type.</span>
                                      ) : (
                                      ...
                                      <div key={a.id} className="field" style={{ margin: 0, flex: a.dataType === 'POSITION' ? '1 1 100%' : '1 1 160px', minWidth: 120 }}>
                                        ...
                                        {a.dataType === 'POSITION' ? (() => {
                                          ...
                                          <input
                                            ...
                                            onChange={e => setEditLinkAttrs(prev => ({ ...prev, [a.name]: e.target.value }))}
                    ...
                    <tbody>
                      {pGroupMap.get(ltName).map(p => {
                        ...
                        return (
                          ...
                          <tr
                            className={isParentSelected ? 'link-selected' : ''}
                            style={{ cursor: 'pointer' }}
                            onClick={() => setSelectedParentLinkId(id => id === p.linkId ? null : p.linkId)}
                          >
                            <td style={{ fontFamily: 'var(--sans)', fontSize: 12 }}>
                              {p.linkLogicalId
          ...
          <div className="history-lc-section">
            ...
            <LifecycleDiagram
              ...
              availableTransitionNames={new Set(
                transitions.filter(a => !a.guardViolations?.length).map(a => a.label)
              )}
              ...
              onTransition={lcTransition => {
                ...
                const action = transitions.find(a => a.label === tName);
